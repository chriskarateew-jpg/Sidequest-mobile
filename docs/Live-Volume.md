...
**Per local-challenges generation batch** (`local-challenges.ts`, one Claude call per batch of up to 8 venues, cached 7 days per ~1km region, gated 5 generations/user/day): ~750 input + up to 800 output ≈ **$0.0033/batch**, and this only fires on a cache miss — negligible in aggregate. The file's negative-cache-for-empty-results is intentionally short (1 hour, `EMPTY_NEGATIVE_CACHE_SECONDS`), and `city-image.ts`'s own permanent-negative-cache bug (the NYC incident referenced in AGENTS.md) is confirmed still fixed — negative geocode/image lookups now expire after 30 days while positive results cache forever.
